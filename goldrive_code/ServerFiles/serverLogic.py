...
def _handle_messages(main_server, msg_q):
    ...
    recv_commands = {"01": _handle_registration, "02": _handle_login, "05": _handle_change_email,
                     ...
                     "26": _handle_forgot_password, "27": _handle_zip_folder, "28": _handle_disconnect}
    ...
    while True:
        ...
        if protocol_num == "02" or protocol_num == "09" or protocol_num == "23" or protocol_num == "28":
...
def _handle_login(main_server, db, client_ip, users_by_ip, code_dic, username, password):
    ...
    if encryption.hash_msg(password) == db.get_password(username) and not any(username == user
                                                                              for user in users_by_ip.values()):
        status = 0
...
def _handle_download_file(main_server, client_ip, path, selected_path):
    ...
    :param selected_path: the path where the user wants to download the file into
...
def _send_email(main_server, my_db, client_ip, code_dic, email):
    ...
    # the verify code
    verify_code = secrets.SystemRandom().randint(100000, 999999)
...
def _handle_disconnect(main_server, db, client_ip, users_by_ip):
    """
    :param main_server: the server object
    :param db: the database
    :param client_ip: the clients ip
    :param users_by_ip: the dict containing each user by the ip it connected on
    :return:
    """
    if client_ip in users_by_ip:
        del users_by_ip[client_ip]
